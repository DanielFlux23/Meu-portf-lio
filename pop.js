...
mover(bloco, config = {}) {
  ...
  let estado = {
    x: elemento.offsetLeft, // Posição inicial real do elemento
  ...
  Object.keys(config).forEach((chave) => {
    ...
    // Definir valores padrão
    obj.x = obj.x ?? estado.x;
    obj.y = obj.y ?? estado.y;
    obj.ax = obj.ax ?? 0;
    obj.ay = obj.ay ?? 0;
    obj.delay = obj.delay ?? 1000;
    ...
    obj.friccao = obj.friccao ?? 1;
    obj.reset = obj.reset ?? false;
    ...
    setInterval(() => {
      ...
      elemento.style.transform = `translate(${estado.x - elemento.offsetLeft}px, ${estado.y - elemento.offsetTop}px)`;
      
      // Reseta a posição após 3 segundos se "reset" for true
      if (obj.reset) {
        setTimeout(() => {
          estado.x = elemento.offsetLeft;
          estado.y = elemento.offsetTop;
...
style(bloco){
 ...
 evento(seletor, typeEvento, funcao) {
  ...
  $$(elemento) {
   ...
   classe(classe, elementos) {
  // Verifica se 'elementos' é um array ou uma lista de elementos
  for (let i = 0; i < elementos.length; i++) {
    let elemento = this.$$(elementos[i]); // Usa querySelector para selecionar o elemento
    if (elemento) {
